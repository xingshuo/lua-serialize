...
// cast to big endian begin
const union {
	int dummy;
	char is_little;
} nativeendian = {1};

union MarshallFloat64 {
	int64_t i;
	double f;
};

inline uint16_t
cast_bigendian_u16(int16_t src) {
	uint16_t dst;
	if (nativeendian.is_little) {
		((uint8_t *)&dst)[0] = ((const uint8_t *)&src)[1];
		((uint8_t *)&dst)[1] = ((const uint8_t *)&src)[0];
	} else {
		dst = src;
	}
	return dst;
...
inline uint32_t
cast_bigendian_u32(int32_t src) {
	uint32_t dst;
	if (nativeendian.is_little) {
...
inline uint64_t
cast_bigendian_u64(int64_t src) {
    uint64_t dst;
    if (nativeendian.is_little) {
...
wb_integer(struct write_block *wb, lua_Integer v) {
	...
	if (v == 0) {
		...
		uint64_t v64 = cast_bigendian_u64(v);
		wb_push(wb, &n, 1);
		wb_push(wb, &v64, sizeof(v64));
	} else if (v < 0) {
		uint32_t v32 = cast_bigendian_u32(v);
		...
		wb_push(wb, &n, 1);
		uint16_t word = cast_bigendian_u16(v);
...
wb_real(struct write_block *wb, double v) {
	...
	wb_push(wb, &n, 1);
	union MarshallFloat64 mf;
	mf.f = v;
	uint64_t v64 = cast_bigendian_u64(mf.i);
	wb_push(wb, &v64, sizeof(v64));
}
...
wb_string(struct write_block *wb, const char *str, int len) {
	if (len < MAX_COOKIE) {
		...
		if (len < 0x10000) {
			...
			wb_push(wb, &n, 1);
			uint16_t x = cast_bigendian_u16(len);
			wb_push(wb, &x, 2);
		} else {
			n = COMBINE_TYPE(TYPE_LONG_STRING, 4);
			wb_push(wb, &n, 1);
			uint32_t x = cast_bigendian_u32(len);
			wb_push(wb, &x, 4);
		}
		wb_push(wb, str, len);
	}
